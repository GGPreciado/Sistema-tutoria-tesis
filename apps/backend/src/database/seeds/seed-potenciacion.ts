...
async function main(): Promise<void> {
  ...
  try {
    const seedsPath = path.join(__dirname, '../../../../../docs/seeds-potenciacion.json');
    const data: PotenciacionData = JSON.parse(fs.readFileSync(seedsPath, 'utf-8'));
    ...
    const porNivel = await qr.query(`
      ...
      GROUP BY dificultad
      ORDER BY dificultad
    `, [temaIdMap.get('potenciacion')]);
    console.log('\nPreguntas de Potenciación por nivel de dificultad:');
